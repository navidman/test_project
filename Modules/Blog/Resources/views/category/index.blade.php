                                ...
                                <div class="col-12 field-style">
                                    <select data-placeholder="یک مورد را انتخاب کنید..." class="form-control chosen-rtl select" name="parent" id="parent">
                                        ...
                                        @forelse($ParentCategories as $item)
                                            <option value="{{ $item->id }}" @selected(old('parent') == $item->id)>{{ $item->title }}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
                ...
                @if(count($BlogCategory))
                    <div class="widget-block widget-item widget-style">
                        ...
                        <div class="widget-content">
                            <form action="{{ route('blog-category.destroy') }}" method="post" onsubmit="return confirm('<?php echo "آیا از حذف موارد انتخاب شده مطمئن هستید؟";?>');">
                                @csrf
                                <table class="table align-items-center">
                                    ...
                                    @foreach ($BlogCategory as $item)
                                        <tr>
                                            ...
                                            <td class="center num-fa">{{ count($item->blog_tbl()->get()) }}</td>
                                            @can(['isAuthor'])
                                                <td class="center">
                                                    <a href="{{ route('blog-category.edit', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-edit"></i></a>
                                                </td>
                                            @endcan
                                        </tr>
                                    ...
                                    <tr>
                                        <td style="vertical-align: middle;" colspan="8">
                                            <div class="row align-items-center no-gutters">
                                                <div class="col-2">
                                                    {{ $BlogCategory->total() }} مورد
                                                </div>
                                                <div class="col-10 left">
                                                    <div class="pagination-table">
                                                        {{$BlogCategory->appends(request()->input())->links('vendor.pagination.default')}}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
