            ...
            <div class="col-4 create-col form-section">
                <div class="widget-block widget-item widget-style">
                    <div class="heading-widget">
                        <span class="widget-title">افزودن گروه شغلی</span>
                    </div>
                    <div class="widget-content widget-content-padding">
                        <form action="{{ route('advertisement-category.store') }}" method="POST">
                            @csrf
                            <div class="form-group row no-gutters">
                                @if($errors->has('title'))
                                    <span class="col-12 message-show">{{ $errors->first('title') }}</span>
                                @endif
                                {!! Form::text('title',null,[ 'id'=>'title' , 'class'=>'col-12 field-style input-text', 'placeholder'=>'عنوان گروه شغلی را وارد نمایید']) !!}
                            ...
                            <div class="form-group row no-gutters">
                                ...
                                @endif
                                {!! Form::text('slug',null,[ 'id'=>'slug' , 'class'=>'col-12 field-style input-text', 'placeholder'=>'شناسه گروه شغلی را وارد نمایید']) !!}
                                {!! Form::label('slug','شناسه گروه شغلی:',['class'=>'col-12']) !!}
                            </div>
                            <button type="submit" class="submit-form-btn ">افزودن گروه شغلی</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="col-8">
                @if(count($EmploymentAdvertisementCategory))
                    <div class="widget-block widget-item widget-style">
                        <div class="heading-widget">
                            <div class="row align-items-center">
                                <div class="col-12">
                                    <span class="widget-title">گروه های شغلی</span>
                                </div>
                            </div>
                        </div>

                        <div class="widget-content">
                            <form action="{{ route('advertisement-category.destroy') }}" method="post" onsubmit="return confirm('<?php echo "آیا از حذف موارد انتخاب شده مطمئن هستید؟";?>');">
                                @csrf
                                <table class="table align-items-center">
                                    ...
                                    <tbody>
                                    @foreach ($EmploymentAdvertisementCategory as $item)
                                        <tr>
                                            <td class="delete-col">
                                            ...
                                            @can(['isAuthor'])
                                                <td class="center">
                                                    <a href="{{ route('advertisement-category.edit', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-edit"></i></a>
                                                </td>
                                            @endcan
                                        </tr>
                                    @endforeach
                                    ...
                                    <tr>
                                        <td style="vertical-align: middle;" colspan="8">
                                            <div class="row align-items-center no-gutters">
                                                <div class="col-2">
                                                    {{ $EmploymentAdvertisementCategory->total() }} مورد
                                                </div>
                                                <div class="col-10 left">
                                                    <div class="pagination-table">
                                                        {{ $EmploymentAdvertisementCategory->appends(request()->input())->links('vendor.pagination.default') }}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
