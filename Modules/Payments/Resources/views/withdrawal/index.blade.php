...
@section('title-page')
    ...
    <span class="text">درخواست های تسویه حساب</span>
@endsection
        ...
        @if(count($Withdrawal))
            <div class="widget-block widget-item widget-style">
                    ...
                    <div class="row align-items-center">
                        <div class="col-10">
                            <div class="form-style small-filter">
                                <span class="widget-title">درخواست ها</span>
                ...
                <div class="widget-content">
                    <form action="{{ url('dashboard/withdrawal/destroy') }}" method="post" onsubmit="return confirm('<?php echo "آیا از حذف موارد انتخاب شده مطمئن هستید؟";?>');">
                        @csrf
                        <table class="table align-items-center">
                            ...
                            @foreach ($Withdrawal as $item)
                                <tr @class(['cursor-pointer','new-record' => $item->status == 'new'])>
                                    @can('isAuthor')
                                        <td class="delete-col">
                                            <input class="delete-checkbox" type="checkbox" name="delete_item[{{ $item->id }}]" value="1">
                                        </td>
                                    @endcan
                                    <td class="cursor-pointer" onclick="window.location.href = '{{ url('dashboard/withdrawal/' . $item->id) }}/edit'">{{ $item->user['name'] }}</td>
                                    <td class="num-fa">{{ number_format($item->amount) }} تومان</td>
                                    <td>
                                        @switch($item->status)
                                            @case('new')
                                            <span style="color: #0066ff">جدید</span>
                                            @break
                                            @case('rejected')
                                            <span style="color: red">رد شد</span>
                                            @break
                                            @case('accepted')
                                            <span style="color: #009912">تایید شد</span>
                                            @break
                                        @endswitch
                                    </td>
                                    ...
                                    @can('isAuthor')
                                        <td class="center">
                                            <a href="{{ route('withdrawal.edit', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-eye"></i></a>
                                        </td>
                                    @endcan
                                </tr>
                            @endforeach
                            ...
                            <tr>
                                <td style="vertical-align: middle;" colspan="20">
                                    <div class="row align-items-center">
                                        <div class="col-4">
                                            نمایش موارد {{ $Withdrawal->firstItem() }} تا {{ $Withdrawal->lastItem() }}
                                            از {{ $Withdrawal->total() }} مورد (صفحه {{ $Withdrawal->currentPage() }}
                                            از {{ $Withdrawal->lastPage() }})
                                        </div>
                                        <div class="col-8 left">
                                            <div class="pagination-table">
                                                {{$Withdrawal->links('vendor.pagination.default')}}
                                            </div>
                                        </div>
                                    </div>
                                </td>
