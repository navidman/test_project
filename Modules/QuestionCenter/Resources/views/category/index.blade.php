...
@section('title-page')
    ...
    <span class="text">مدیریت دسته بندی ها</span>
            ...
            <div class="col-4 create-col form-section">
                <div class="widget-block widget-item widget-style">
                    <div class="heading-widget">
                        <span class="widget-title">افزودن دسته بندی جدید</span>
                    </div>
                    <div class="widget-content widget-content-padding">
                        <form action="{{ route('question-center-category.store') }}" method="POST">
                            @csrf
                            <div class="form-group row no-gutters">
                                @if($errors->has('title'))
                                    <span class="col-12 message-show">{{ $errors->first('title') }}</span>
                                @endif
                                {!! Form::text('title',null,[ 'id'=>'title' , 'class'=>'col-12 field-style input-text', 'placeholder'=>'عنوان دسته بندی را وارد نمایید']) !!}
                            ...
                            <div class="form-group row no-gutters">
                                ...
                                @endif
                                {!! Form::text('slug',null,[ 'id'=>'slug' , 'class'=>'col-12 field-style input-text', 'placeholder'=>'شناسه دسته بندی را وارد نمایید']) !!}
                                {!! Form::label('slug','شناسه دسته بندی:',['class'=>'col-12']) !!}
                            </div>
                            <button type="submit" class="submit-form-btn ">افزودن دسته بندی جدید</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="col-8">
                @if(count($QuestionCenterCategory))
                    <div class="widget-block widget-item widget-style">
                        <div class="heading-widget">
                            <div class="row align-items-center">
                                <div class="col-12">
                                    <span class="widget-title">دسته بندی ها</span>
                                </div>
                            </div>
                        </div>

                        <div class="widget-content">
                            <form action="{{ route('question-center-category.destroy') }}" method="post" onsubmit="return confirm('<?php echo "آیا از حذف موارد انتخاب شده مطمئن هستید؟";?>');">
                                @csrf
                                <table class="table align-items-center">
                                    ...
                                    <tbody>
                                    @foreach ($QuestionCenterCategory as $item)
                                        <tr>
                                            <td class="delete-col">
                                            ...
                                            <td class="center num-fa">{{ count($item->question_tbl()->get()) }}</td>
                                            @can(['isAuthor'])
                                                <td class="center">
                                                    <a href="{{ route('question-center-category.edit', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-edit"></i></a>
                                                </td>
                                            @endcan
                                        </tr>
                                    @endforeach
                                    ...
                                    <tr>
                                        <td style="vertical-align: middle;" colspan="8">
                                            <div class="row align-items-center no-gutters">
                                                <div class="col-2">
                                                    {{ $QuestionCenterCategory->total() }} مورد
                                                </div>
                                                <div class="col-10 left">
                                                    <div class="pagination-table">
                                                        {{$QuestionCenterCategory->appends(request()->input())->links('vendor.pagination.default')}}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
