                ...
                <div class="widget-content">
                    <form action="{{ url('dashboard/request-center/destroy') }}" method="post" onsubmit="return confirm('<?php echo "آیا از حذف موارد انتخاب شده مطمئن هستید؟";?>');">
                        ...
                        <table class="table align-items-center">
                            ...
                            <tbody>
                            @foreach ($RequestReceived as $item)
                                <tr @class(['new-record' => $item->status == 'new' || $item->status == 'replied'])>
                                    @can('isAuthor')
                                        <td class="delete-col">
                                    ...
                                    @endcan
                                    <td>{{ $item->id }}</td>
                                    <td>{{ \App\Http\Controllers\HomeController::TruncateString($item->title, 120, 1) }}</td>
                                    ...
                                    <td class="center">
                                        <a href="{{ route('request-replies.show', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-eye"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            ...
                            <tr>
                                <td style="vertical-align: middle;" colspan="20">
                                    <div class="row align-items-center">
                                        <div class="col-4">
                                            نمایش موارد {{ $RequestReceived->firstItem() }} تا {{ $RequestReceived->lastItem() }}
                                            از {{ $RequestReceived->total() }} مورد (صفحه {{ $RequestReceived->currentPage() }}
                                            از {{ $RequestReceived->lastPage() }})
                                        </div>
                                        <div class="col-8 left">
                                            <div class="pagination-table">
                                                {{$RequestReceived->links('vendor.pagination.default')}}
                                            </div>
                                        </div>
                                    </div>
                                </td>
